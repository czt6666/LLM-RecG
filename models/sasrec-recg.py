...
class SASRecWithDomainAlignment(nn.Module):
    """
    SASRec with dynamic pretrained embeddings, domain alignment projections, and sequential pattern generalization.
    """
    ...
    def forward(self, item_seq: torch.LongTensor, is_target_domain: bool = False) -> torch.Tensor:
        ...
        # causal mask for SASRec (different from BERT4Rec)
    ...
    def extract_sequential_patterns_from_source(self, source_sequences):
        ...
        with torch.no_grad():
            ...
            # get user representation from last position
